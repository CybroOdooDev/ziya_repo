...
class LinkedInAccount(models.Model):
    ...
    def action_sync_organizations(self):
        ...
        if self.last_sync_orgs and (datetime.now() - self.last_sync_orgs).total_seconds() < 30:
            _logger.info(f"Skipping organization sync for {self.name} (last sync was {self.last_sync_orgs})")
            return self.env['linkedin.organization'].search([('account_id', '=', self.id), ('state', '=', 'active')])
            ...
            if user_res.ok:
                ...
                if person_sub:
                    ...
                    if existing_person:
                        ...
                        vals.update({'org_urn': person_urn, 'account_id': self.id})
                        new_person = self.env['linkedin.organization'].create(vals)
    ...
    def _fetch_feeds_for_urn(self, owner_urn, author_name, profile_pic_url, legacy_headers, org_id=False, start=0, count=15):
        ...
        _logger.info(f'Fetching LinkedIn Posts for {owner_urn}: GET {url} (count={count})')

        try:
            response = requests.get(url, params=params, headers=rest_headers, timeout=30)
            _logger.info(f'LinkedIn Posts API Response: {response.status_code}')
            if not response.ok:
                _logger.error(f'LinkedIn Posts API failed: {response.status_code} - {response.text[:300]}')
            ...
            for element in elements:
                ...
                if existing_feed and existing_feed.last_sync_social_actions:
                    now = datetime.now()
                    diff_hours = (now - existing_feed.last_sync_social_actions).total_seconds() / 30
                    post_age_days = (now - existing_feed.posted_date).days if existing_feed.posted_date else 0
                ...
                if content:
                    ...
                    if media and 'id' in media: # It might be an asset URN
                        # rest/posts usually has thumbnails or we have to fetch the asset
                        # For now, let's look for media if it's directly there
                        pass
                    ...
                    if poll_data:
                        ...
                        poll_options_json = _json.dumps([
                            {'text': o.get('text', ''), 'voteCount': o.get('voteCount', 0)}
                            for o in options
                        ])
                        poll_duration = poll_data.get('settings', {}).get('duration', '')
                        poll_total_votes = poll_data.get('uniqueVotersCount', 0)

                # Published date
                first_pub = element.get('publishedAt') or element.get('createdAt')
                ...
                vals = {
                    ...
                    'last_sync_social_actions': datetime.now() if should_refresh_actions else (existing_feed.last_sync_social_actions if existing_feed else False),
                    'is_poll': is_poll,
            ...
            _logger.exception(f"Exception in _fetch_feeds_for_urn for {owner_urn}: {e}")
            return {'created_count': 0, 'has_more': False}
