...
class SocialMediaPost(models.Model):
    ...
    # ── Poll fields ──────────────────────────────────────
    linkedin_poll_question = fields.Char(string="Poll Question", help="Question for the LinkedIn poll.")
    ...
    def _post_linkedin_image(self, post, org, account, headers):
        ...
        # 1. Register Upload for first image (Handling multiple images is more complex on v2/ugcPosts, so we handle one for now)
        # Assuming we just upload the first image for simplicity, or we can loop if UGC strictly allows multiple
        media_assets = []
        for img in post.linkedin_image_ids:
            ...
            upload_res = requests.put(upload_url, headers=upload_headers, data=image_content, timeout=120)
        ...
        return requests.post(url, headers=headers, json=post_payload, timeout=60)
    ...
    def _post_linkedin_poll(self, post, org, account, headers):
        ...
        # Gather options
        options = []
        for opt in [post.linkedin_poll_option_1, post.linkedin_poll_option_2, post.linkedin_poll_option_3, post.linkedin_poll_option_4]:
            if opt and opt.strip():
        ...
        return requests.post(url, headers=rest_headers, json=payload, timeout=60)
